...
async def extract_spec(data, header, x, y, filename):
    ...
    freq = freq_start + np.arange(len(flux)) * freq_step
    ...
    with fits.open(ref_input_data, memmap=True, mode='denywrite') as hdul:
        ...
        for c in catalogs:
            ...
            for row in table:
                ...
                task = asyncio.create_task(extract_spec(data, header, x, y, spec_file))
                tasks.append(task)
