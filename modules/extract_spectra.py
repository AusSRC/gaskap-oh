...
async def extract_spec(detection_id, data, df, x, y, conn):
    ...
    logging.info(
        "(%.2f s) Updated spec product for detection %i"
        % (time.time() - start, detection_id)
    )
    return
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            instance = await conn.fetchrow(
                "SELECT * FROM instance WHERE run_id = $1 LIMIT 1", run["id"]
            )
            assert instance is not None, "Instance does not exist"
            instance = dict(instance)
            params = eval(instance["parameters"])
            image_cube_file = params["input.data"]
            assert os.path.exists(image_cube_file), (
                "Image cube %s does not exist" % image_cube_file
            )
