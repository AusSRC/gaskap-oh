...
async def sidelobe_plots(conn, maser, detection, sidelobes):
    ...
    ax1.plot(
        detection_spec["vel"],
        detection_spec["f_peak"] / detection["rms"],
        color="red",
        linewidth=2,
    )
...
async def maser_sidelobe_filter(
    conn, detections, snr_threshold, second_peak_channels, second_peak_snr
):
    ...
    """
    logging.info(
        "Maser-sidelobe filter iteration with snr threshold = %i" % snr_threshold
    )
    logging.info(
        "Secondary peak filter using difference %i from 2nd peak with snr = %.2f"
        % (second_peak_channels, second_peak_snr)
    )
    ...
    for idx, maser in masers_df.iterrows():
        ...
        for _, detection in unresolved_df.iterrows():
            ...
            if abs(peak_z - d_peak_z) < 2:
                ...
                if (abs(peak_z_2 - d_peak_z) > second_peak_channels) & (
                    (f_peak_2 / rms) > second_peak_snr
                ):
                    continue
        ...
        logging.info(
            "Maser %s: matched %i sidelobes. %i remaining unresolved detections"
            % (name, len(sidelobe_d_ids), len(unresolved_df))
        )
...
async def main(argv):
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            masers_df, reject_df, unresolved_df = await maser_sidelobe_filter(
                conn, detections_df, 100, 50, 10
            )
            logging.info(
            ...
            logging.info(
                "SNR filter (round 2): removed %i unresolved detections"
                % len(unresolved_df[unresolved_df["snr"] < 4])
            )
            logging.info(
                "Accepted: %i, rejected: %i, unresolved: %i\n"
                % (len(masers_df), len(reject_df), len(unresolved_df))
            ...
            masers_df_iter2, reject_df_iter2, unresolved_df_iter2 = (
                await maser_sidelobe_filter(conn, unresolved_df, 10, 50, 5)
            )
